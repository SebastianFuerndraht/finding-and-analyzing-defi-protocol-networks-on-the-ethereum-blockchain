...
# make sure to set the path to the correct json file where you can specify which protocols should be plotted per category
categories <- fromJSON(file = "./data/input/protocols-to-compare-per-category.json")

# directory where the plots will be saved
PATH_PLOT_DIR <- "./results/plots/categories"
...
# centrality used for node sizes
...
for (protocols in categories) {
  ...
  mfrow <- c(ceiling(protocol_count/2), 2)
  mar <- c(1, 1, 1, 1)
  mar <- mar * 0.2
  ...
  cex_size <- size/cex_size_dividor

  if (FORMAT_PLOT_FILE=="svg"){
    svg(paste0(PATH_PLOT_DIR, "/svg/", names[count], ".svg"), width = size*4, height = size*4 * ratio)
  }
  if (FORMAT_PLOT_FILE=="png"){
    png(paste0(PATH_PLOT_DIR, "/png/", names[count], ".png"), width = size*4, height = size*4 * ratio, units="in", res=100)
  }
  if (FORMAT_PLOT_FILE=="pdf") {
    pdf(file= paste0(PATH_PLOT_DIR, "/pdf/", names[count], ".pdf"), width = size, height = size * ratio)
  }
  ...
  for (protocol in protocols) {
    ...
    plot(simplify(g), layout=layout_algo, margin=mar,
         ...
         edge.width=edge_width, edge.color=rgb(190/255,190/255,190/255,0.5) , asp=0.707)

    title(main=protocol, cex.main=cex_size*1.3, line=-10)
    ...
    a <- legend('bottomright',legend=top_node_labels,cex=cex_size, col='white',
                pch=21, pt.bg='white', title="Nodes by PR", y.intersp=2, bty="n")
    x <- (a$text$x + a$rect$left) / 2.05
    y <- a$text$y
